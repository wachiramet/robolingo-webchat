...
try {
  ...
  await page
    .getByRole("log")
    .getByText("ข้อความทดสอบ <script>alert(1)</script>", { exact: true })
    .waitFor();
  assert.equal(await editor.inputValue(), "");
  await page.getByRole("button", { name: /นนท์ ธนกร/ }).click();
  ...
  await page
    .getByRole("log")
    .getByText("ขอบคุณค่ะ ขอรายละเอียดเพิ่มเติมได้ไหมคะ", { exact: true })
    .waitFor();
  await page.reload();
  await page.getByText("โหมดทดลอง", { exact: true }).waitFor();
  ...
  await page.screenshot({
    path: ".impeccable/review/mobile-chat.png",
    fullPage: true,
  });
  assert.ok(
    await page.evaluate(
  ...
  console.log(
    "PASS: search, draft isolation, sending, safe text rendering, incoming demo, reset, mobile navigation, 320/390/768 overflow, no page errors.",
  );
} finally {
  await browser.close();
}
